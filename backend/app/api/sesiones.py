...
from app.schemas.sesion_schema import AgendarSesionRequest, SesionOut
...
async def agendar_sesion(
    ...
    if duracion_horas > horas_restantes:
        raise HTTPException(
            ...
            f"Horas insuficientes en el contrato. Disponibles: {horas_restantes:.1f}h, solicitadas: {duracion_horas:.1f}h",
        )
    ...
    if not disponibilidad:
        raise HTTPException(
            ...
            "El mentor no tiene disponibilidad en ese bloque horario",
        )
    ...
    if res_colision.scalars().first():
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            "El mentor ya tiene una sesion en ese horario (double-booking detectado)",
        )
